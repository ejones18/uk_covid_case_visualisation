"""
nhs_covid_data_visualiser module for localised COVID data visualiser - plots a graph using matplot.
...
def add_to_geo_json_eer(data, geo_json, caseType):
    """
    - Adds the COVID data the corresponding EER region in the geo_json file.
    """
    for region in geo_json['features']:
        try:
            ...
            region['properties'][caseType] = eer_list
        except:
            region['properties'][caseType] = [0]
    return geo_json

def add_to_geo_json_lad(data, geo_json, caseType):
    """
    - Adds the COVID data the corresponding LAD region in the geo_json file.
    """
    for region in geo_json['features']:
        try:
            ...
            region['properties'][caseType] = lad_list
        except:
            region['properties'][caseType] = [0]
    return geo_json
...
def plot_data(data):
    """
    - Plots the data using matlabplot.
    """
    # takes data frame (data) with regions as columns and dates as rows and plots all of the data
...
def get_all_region_data(data, cumulative=True):
    """
    - Fetches all regional data. Returns dataframe.
    """
    regional_data = {}
    region_names = set()
    for i in data['data']:
...
def get_regional_data(offline=False):
    """
    - Fetches json file if timestamp isn't the same as new json data.
    """
    try:
        with open('regional_covid_data.json', 'r') as json_file:
            ...
            time_created = os.path.getctime(path_to_regional_file)
...
def get_ltla_data(offline=False):
    ...
    try:
        with open('ltlas_covid_data.json', 'r') as json_file:
            ...
            time_created = os.path.getctime(path_to_ltla_file)
            time_created = datetime.datetime.fromtimestamp(time_created)
            current_time = datetime.datetime.now()
            delta_t = (current_time - time_created)
            time_out = datetime.timedelta(days=2)
...
def parse_options():
    ...
    parser.add_argument("-D", "--delta", dest="delta", action="store_false",
                        required=False,
                        help="Reads number of cases that day.")
